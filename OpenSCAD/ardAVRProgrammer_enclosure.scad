...
module lidold() {
  ...
  translate([0, 0, 0])
    ...
    union() {
      color("pink")
	cube([ardWidth + extraWidthL + extraWidthR + casWallThick * 2 - radius * 2, 
	      ardDepth + extraDepthD + extraDepthU + casWallThick * 2 - radius * 2, 
	      lidThick], center = true);
      
      // bottom
      translate([0, 
		 -(ardDepth + extraDepthD + extraDepthU + casWallThick * 2) / 2 + radius, 
		 radius / 2])
	rotate([0, 90, 0])
	cylinder(h = ardWidth + extraWidthL + extraWidthR + casWallThick * 2, 
		 r1 = radius, r2 = radius, 
		 center = true, $fn = roundness); 
	...
	cylinder(h = ardWidth + extraWidthL + extraWidthR + casWallThick * 2, 
      ...
      // left
      translate([-(ardWidth + extraWidthL + extraWidthR + casWallThick * 2) / 2 + radius, 
		 0, 
		 radius / 2])
	rotate([90, 0, 0])
	cylinder(h = ardDepth + extraDepthD + extraDepthU + casWallThick * 2, 
		 r1 = radius, r2 = radius, 
		 center = true, $fn = roundness);
    ...
    translate([0, 0, lidThick])
      color("red")
      cube([ardWidth + extraWidthL + extraWidthR + casWallThick * 2, 
	    ardDepth + extraDepthD + extraDepthU + casWallThick * 2, lidThick], 
	   center = true);
  }
