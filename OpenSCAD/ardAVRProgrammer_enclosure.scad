...
module bottomCasing() {
  ...
  union() {
	...
	cube([ardWidth + extraWidthL + extraWidthR, ardDepth + extraDepthD + extraDepthU, soBotHeight + soTopHeight + ardHeight + extraHeight]);
      ...
      // usb port
      translate([0, 30.2 + casWallThick + extraDepthD, casWallThick + soBotHeight + soTopHeight])
	color("red")
...
module lid() {
  ...
  difference() {
	...
	translate([casWallThick + tolerance, casWallThick + tolerance, -lidInset + tolerance])
	  color("green")
	  cube([ardWidth + extraWidthL + extraWidthR + casWallThick * 2 -casWallThick * 2 - tolerance * 2, ardDepth + extraDepthD + extraDepthU + casWallThick * 2 -casWallThick * 2 - tolerance * 2, lidInset - tolerance]);
	
	translate([casWallThick + tolerance + lidRidgeWidth, casWallThick + tolerance + lidRidgeWidth, -lidInset + tolerance])
	  color("red")
	  cube([ardWidth + extraWidthL + extraWidthR + casWallThick * 2 -casWallThick * 2 - lidRidgeWidth* 2 - tolerance * 2, ardDepth + extraDepthD + extraDepthU + casWallThick * 2 -casWallThick * 2 - lidRidgeWidth * 2 - tolerance * 2, lidInset - tolerance]);
      }
     lidOpenings(); 
    }
  ...
  // screw holes
  for (i = [0 : 1 : 3]) {
    translate([extraWidthL + casWallThick, extraDepthD + casWallThick, -postsY - lidThick / 2])
    translate(lidPostsPos[i])
      cylinder(h = lidThick + lidInset, r1 = lidHoleDia / 2, r2 = lidHoleDia / 2, center = true, $fn = roundness);
    }
